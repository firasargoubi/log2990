...
describe('CombatComponent', () => {
    ...
    it('should display a notification when the combat ends', () => {
        ...
        expect(mockNotificationService.showInfo).toHaveBeenCalledWith('Le combat est terminé! Joueur 2 a perdu !');
    });
    ...
    it('should show "Vous avez perdu" notification when current player loses', () => {
        const losingPlayer = { ...component.currentPlayer };
        combatEndedSubject.next({ loser: losingPlayer });

        component.ngOnInit();
        fixture.detectChanges();

        expect(mockNotificationService.showInfo).toHaveBeenCalledWith('Le combat est terminé! Vous avez perdu !');
    });

    it('should show opponent lost notification when opponent loses', () => {
        const opponent = { ...component.opponent };
        combatEndedSubject.next({ loser: opponent });

        component.ngOnInit();
        fixture.detectChanges();

        expect(mockNotificationService.showInfo).toHaveBeenCalledWith(`Le combat est terminé! ${opponent.name} a perdu !`);
    });
