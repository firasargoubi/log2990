...
describe('BoardService', () => {
    ...
    describe('handleTeleport', () => {
        it('should return unchanged state if current player is not found', () => {
            const gameState = {
                players: [{ id: 'p1' }],
                currentPlayer: 'unknown',
                playerPositions: [{ x: 0, y: 0 }],
            ...
            const result = boardService.handleTeleport(gameState, { x: 1, y: 1 });
            expect(result).to.equal(gameState);
        });

        it('should return unchanged state if player position is undefined', () => {
            const gameState = {
                players: [{ id: 'p1' }],
                currentPlayer: 'p1',
                playerPositions: [],
        ...
        it('should return unchanged state if target coordinate is occupied by another player', () => {
            const gameState = {
                players: [{ id: 'p1' }, { id: 'p2' }],
                currentPlayer: 'p1',
                playerPositions: [
        ...
        it('should teleport the player and update moves when movement points are non-negative', () => {
            const gameState = {
                players: [{ id: 'p1', speed: 2, bonus: {} }],
                currentPlayer: 'p1',
                playerPositions: [{ x: 0, y: 0 }],
            ...
            sandbox.stub(boardService as any, 'findAllPaths').returns([{ x: 2, y: 2 }]);
            sandbox.stub(boardService as any, 'calculateShortestMoves').returns([
                [
                    { x: 1, y: 1 },
                    { x: 2, y: 2 },
                ],
            ...
            const targetCoordinate = { x: 1, y: 1 };
            const result = boardService.handleTeleport(gameState, targetCoordinate);
            expect(result.playerPositions[0]).to.deep.equal(targetCoordinate);
            expect(result.availableMoves).to.deep.equal([{ x: 2, y: 2 }]);
            expect(result.shortestMoves).to.deep.equal([
                [
                    { x: 1, y: 1 },
                    { x: 2, y: 2 },
        ...
        it('should teleport the player and clear moves when movement points are negative', () => {
            const gameState = {
                players: [{ id: 'p1', speed: 2, bonus: {} }],
                currentPlayer: 'p1',
                playerPositions: [{ x: 0, y: 0 }],
    ...
    describe('isOccupied', () => {
        it('should return true if the target coordinate is occupied by another player', () => {
            const gameState = {
                board: [
                    [0, 0],
                    [0, 0],
        ...
        it('should return false if the target coordinate is free (no player occupies it and board is clear)', () => {
            const gameState = {
                board: [
                    [0, 0],
                    [0, 0],
        ...
        it('should return true if the board tile indicates a blocked item (non-empty and non-spawn)', () => {
            const gameState = {
                board: [
                    [20, 0],
                    [0, 0],
            ...
            const result = (boardService as any).isOccupied(gameState, { x: 0, y: 0 }, 0);
            expect(result).to.equal(true);
        });

        it('should return true if the board tile indicates a wall', () => {
            const gameState = {
                board: [
                    [TileTypes.Wall, 0],
                    [0, 0],
        ...
        it('should return true if the board tile indicates a closed door', () => {
            const gameState = {
                board: [
